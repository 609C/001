...
public class NotificationView extends LinearLayout {
    ...
    protected void onFinishInflate() {
        ...
        this.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (currentUserId != 0 || currentMeetId != 0) 
                {	
                    NotificationCenter.getInstance().addToMemCache("push_user_id", currentUserId);
                    NotificationCenter.getInstance().addToMemCache("push_meet_id", currentMeetId);//wangxm add for meetAlert
                }
                if (currentEncId != 0) {
    ...
    public void show(MessageObject object) {
        ...
        if (chat != null) {
            ...
            nameTextView.setText(Utilities.formatName(user.first_name, user.last_name) + " @ " + chatTitle);
        } else {
            int lower_id = (int)dialog_id;
            if (lower_id != 0 || dialog_id == 0) {
                currentUserId = user.id;
                currentEncId = 0;
                if (object.messageOwner.is_from_other) {
                	currentMeetId = object.messageOwner.id;
				} else {
					currentMeetId = 0;
				}
            } else {
