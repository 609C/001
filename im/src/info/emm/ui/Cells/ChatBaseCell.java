...
public class ChatBaseCell extends BaseCell {
    ...
    protected MRect backGroudRect; // left right top bottom  ->  x y w h
    ...
    public void setMessageObject(MessageObject messageObject) {
        ...
        if (drawName && chat && currentUser != null && !currentMessageObject.messageOwner.out) {
            currentNameString = StringUtil.getUserName(currentUser);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (currentMessageObject.messageOwner.out) {
            ...
            }//layoutWidth - backgroundWidth - Utilities.dp(52) wangxm update
            backGroudRect.setMRect(
            layoutWidth - backgroundWidth,
            Utilities.dp(1),
            backgroundWidth,
            layoutHeight - Utilities.dp(2));
        ...
        setDrawableBounds(currentBackgroundDrawable,  backGroudRect.x,  backGroudRect.y,  backGroudRect.w,  backGroudRect.h);
        currentBackgroundDrawable.draw(canvas);

        if (drawName && nameLayout != null) {
            canvas.save();
            if (currentMessageObject.messageOwner.media.audio != null) {
            	float x = currentBackgroundDrawable.getBounds().left + Utilities.dp(19) + Utilities.dp(50) - nameOffsetX;
            	canvas.translate(x, Utilities.dp(6));
			} else if (currentMessageObject.messageOwner.media.alert != null){
				float x = currentBackgroundDrawable.getBounds().left + Utilities.dp(19) + Utilities.dp(53) - nameOffsetX;
            	canvas.translate(x, Utilities.dp(6));
			}else {
        ...
        if (currentMessageObject.messageOwner.out) {
            ...
            if (drawClock) {
            	int x = layoutWidth - Utilities.dpf(18.5f) - clockDrawable.getIntrinsicWidth();
                setDrawableBounds(clockDrawable, x, layoutHeight - Utilities.dpf(8.5f) - clockDrawable.getIntrinsicHeight());
                clockDrawable.draw(canvas);
            }
            if (drawCheck2) {            	
                if (drawCheck1) {
                	int x = layoutWidth - Utilities.dpf(22.5f) - checkDrawable.getIntrinsicWidth();
                    setDrawableBounds(checkDrawable, x, layoutHeight - Utilities.dpf(8.5f) - checkDrawable.getIntrinsicHeight());
                } else {
                	int x = layoutWidth - Utilities.dpf(18.5f) - checkDrawable.getIntrinsicWidth();
                    setDrawableBounds(checkDrawable, x , layoutHeight - Utilities.dpf(8.5f) - checkDrawable.getIntrinsicHeight());
                }
                checkDrawable.draw(canvas);
            }
            if (drawCheck1) {
            	int x = layoutWidth - Utilities.dp(18) - halfCheckDrawable.getIntrinsicWidth();
                setDrawableBounds(halfCheckDrawable, x, layoutHeight - Utilities.dpf(8.5f) - halfCheckDrawable.getIntrinsicHeight());
                halfCheckDrawable.draw(canvas);
            }
            if (drawError) {
            	int x = layoutWidth - Utilities.dp(18) - errorDrawable.getIntrinsicWidth();
                setDrawableBounds(errorDrawable, x, layoutHeight - Utilities.dpf(6.5f) - errorDrawable.getIntrinsicHeight());
